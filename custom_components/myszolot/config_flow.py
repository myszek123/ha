        ...
        schema_with_defaults = vol.Schema(
            {
                vol.Required(
                    CONF_CHARGER_PHASES,
                    default=current.get(CONF_CHARGER_PHASES, DEFAULT_CHARGER_PHASES),
                ): vol.In([1, 3]),
                vol.Required(
                    CONF_VOLTAGE,
                ...
                vol.Required(
                    CONF_FAST_AMPS,
                    default=current.get(CONF_FAST_AMPS, DEFAULT_FAST_AMPS),
                ): vol.All(vol.Coerce(int), vol.Range(min=1, max=32)),
                vol.Required(
                    CONF_BATTERY_CAPACITY_KWH,
                    default=current.get(CONF_BATTERY_CAPACITY_KWH, DEFAULT_BATTERY_CAPACITY_KWH),
                ): vol.Coerce(float),
                vol.Required(
                    CONF_DEFAULT_TARGET_SOC,
                    default=current.get(CONF_DEFAULT_TARGET_SOC, DEFAULT_TARGET_SOC),
                ...
                vol.Required(
                    ...
                    default=current.get(CONF_CHARGE_START_SOC, DEFAULT_CHARGE_START_SOC),
                ): vol.All(vol.Coerce(int), vol.Range(min=0, max=100)),
                vol.Required(
                    CONF_MAX_PRICE_THRESHOLD,
                    default=current.get(CONF_MAX_PRICE_THRESHOLD, DEFAULT_MAX_PRICE_THRESHOLD),
                ): vol.Coerce(float),
                vol.Required(
                    CONF_SMART_DEADLINE_HOURS,
